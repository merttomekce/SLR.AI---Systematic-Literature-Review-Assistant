      ...
      <div className="p-6 h-[calc(100vh-80px)] flex flex-col overflow-hidden max-w-[1800px] mx-auto animate-in fade-in duration-500 bg-background text-foreground">
        ...
        <div className="flex items-center justify-between mb-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs uppercase tracking-widest font-bold text-muted-foreground">{isRunning ? 'ENGINE ACTIVE' : 'ENGINE STANDBY'}</h2>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Button
        ...
        <div className="flex-1 flex gap-6 min-h-0">
          ...
          <div className="w-[300px] flex-shrink-0 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
            <Card className="bg-card/80 backdrop-blur-xl border border-border rounded-2xl p-6 space-y-6">
              <div>
                <h3 className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-4">Run Metadata</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Total Papers</p>
                    <p className="font-mono text-xl">{stats.total}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Model Snippet</p>
                    <p className="font-mono text-sm text-blue-400 truncate">{currentS1Run?.model || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-1">Run Name</p>
              ...
              <div className="pt-6 border-t border-border">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Progress</h3>
              ...
              <div className="pt-6 border-t border-border">
                <h3 className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-4">Stats</h3>
                ...
                <div className="mt-4 flex items-center justify-between bg-secondary border border-border rounded-xl p-3">
                  <span className="text-[9px] uppercase tracking-widest text-muted-foreground">Inclusion Rate</span>
            ...
            <div className="space-y-3 mt-auto pt-4">
              <Button onClick={handleExport} disabled={!currentS1Run} variant="outline" className="w-full h-10 border-border bg-transparent hover:bg-secondary text-xs">
                <Download className="w-4 h-4 mr-2" /> Export Log
              </Button>
              <Button onClick={handleProceed} disabled={!isComplete || stats.included === 0} className="w-full h-12 gap-2 bg-blue-600 hover:bg-blue-500 text-foreground shadow-[0_0_15px_rgba(37,99,235,0.3)] transition-all text-xs font-bold uppercase tracking-widest group">
          ...
          <div className="flex-1 flex flex-col gap-6 min-w-0">
            ...
            <Card className="flex-shrink-0 bg-card/80 backdrop-blur-xl border border-border rounded-2xl p-8 relative overflow-hidden flex flex-col max-h-[50%]">
              ...
              {selectedPaper ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-4 text-sm text-muted-foreground leading-relaxed max-w-4xl">
                    ...
                    {selectedPaper.s1Reason && (
                      <div className="mt-6 p-4 rounded-xl bg-secondary border border-border border-l-2 border-l-blue-500">
                        <h4 className="text-[10px] uppercase tracking-widest text-muted-foreground mb-2 font-bold flex items-center gap-2"><Info className="w-3 h-3" /> AI Justification</h4>
                        <p className="text-xs text-foreground">{selectedPaper.s1Reason}</p>
                  ...
                  <div className="mt-6 pt-6 border-t border-border flex items-center justify-between">
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Override Decision</p>
                    <Select
                    ...
                    >
                      <SelectTrigger className="w-[180px] h-9 bg-white/[0.03] border border-border text-xs font-mono rounded-lg">
            ...
            {/* Results Sub-grid */}
            <Card className="flex-1 bg-card/80 backdrop-blur-xl border border-border rounded-2xl flex flex-col overflow-hidden min-h-0">
              <div className="p-4 border-b border-border flex items-center justify-between">
                <h3 className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Paper Matrix</h3>
                <div className="flex gap-2">
                  {['All', 'INCLUDED', 'EXCLUDED', 'PENDING', 'ANALYZING'].map(filter => (
                    <button
                      key={filter}
                      onClick={() => setActiveFilter(filter)}
                      className={`px-3 py-1 rounded-md text-[9px] uppercase tracking-widest font-bold transition-all ${activeFilter === filter ? 'bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}
                    >
                      {filter}
                    </button>
                  ))}
                ...
                <div className="grid gap-1">
                  {filteredPapers.map(paper => (
                    <div
                      ...
                      className={`cursor-pointer group flex items-center justify-between p-3 rounded-xl transition-all ${selectedPaperId === paper.id ? 'bg-white/10 border-white/20 shadow-inner' : 'bg-transparent border-transparent hover:bg-secondary flex-row border'} border-solid`}
                    >
                      <div className="flex-1 min-w-0 pr-4">
                        <p className="text-xs font-medium text-foreground truncate">{paper.title}</p>
                        <p className="text-[10px] text-muted-foreground font-mono mt-1 truncate">{paper.author || 'Unknown'}</p>
                      </div>
          ...
          <div className="w-[300px] flex-shrink-0 flex flex-col">
            <Card className="flex-1 bg-card/80 backdrop-blur-xl border border-border rounded-2xl p-4 flex flex-col overflow-hidden">
              <div className="flex items-center gap-2 mb-4 pb-4 border-b border-border">
                <Activity className="w-4 h-4 text-purple-500" />
                <h3 className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Live Screening Log</h3>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 font-mono text-[10px]">
                {papers.slice().reverse().filter(p => p.s1Decision && p.s1Decision !== 'PENDING').slice(0, 100).map(p => (
                  <div key={p.id} className="leading-relaxed border-l-2 border-border pl-3 py-1 cursor-pointer hover:border-white/30 transition-colors" onClick={() => setSelectedPaperId(p.id)}>
                    <div className="text-muted-foreground mb-0.5">[{new Date().toLocaleTimeString()}]</div>
                    <div className="text-foreground line-clamp-2">{p.title}</div>
