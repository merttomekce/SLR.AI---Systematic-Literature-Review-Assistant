...
interface ReviewState {
  ...
  // Local AI Config
  localEngine: 'lmstudio' | 'ollama' | 'jan' | 'custom' | null;
  ...
  setLocalEngine: (engine: 'lmstudio' | 'ollama' | 'jan' | 'custom' | null, endpoint?: string) => void;
...
export const useReviewStore = create<ReviewState>()(
  persist(
    (set, get) => ({
      ...
      localEngine: null,
      customLocalEndpoint: 'http://127.0.0.1:11434/v1',
      ...
      setLocalEngine: (engine, endpoint) => set((state) => ({ 
        localEngine: engine, 
        customLocalEndpoint: endpoint !== undefined ? endpoint : state.customLocalEndpoint 
      })),
      ...
      fetchAvailableModels: async (apiKey: string) => {
        // If it's a local mode flag, use the configured endpoint
        if (apiKey === 'local-mode' || apiKey === 'local-lm-studio') {
          const state = get();
          ...
          try {
            ...
            if (res.ok) {
              ...
              // Handle LM Studio / OpenAI standard `{ data: [...] }`
              if (responseData && Array.isArray(responseData.data)) {
                  models = responseData.data.map((m: any) => m.id || m.name);
              } 
              // Handle Ollama standard `{ models: [...] }`
              else if (responseData && Array.isArray(responseData.models)) {
                  models = responseData.models.map((m: any) => m.name || m.id);
              }
              // Handle raw array fallback
              else if (Array.isArray(responseData)) {
                  models = responseData.map((m: any) => m.id || m.name);
              }
              
              set({ availableModels: models, provider: 'local', model: models[0] || '', isFetchingModels: false });
            } else {
              set({ availableModels: [], isFetchingModels: false });
            }
          } catch (e) {
            console.error(`Failed to connect to local engine at ${endpoint}`, e);
            set({ availableModels: [`Offline (${state.localEngine || 'Local'})`], provider: 'local', model: '', isFetchingModels: false });
          }
          return;
        }
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        customLocalEndpoint: state.customLocalEndpoint,
        model: state.model,
        runName: state.runName,
        topic: state.topic,
        inclusionCriteria: state.inclusionCriteria,
        exclusionCriteria: state.exclusionCriteria,
        extractionFields: state.extractionFields,
        extraContext: state.extraContext,
        papers: state.papers // Save standard papers so refs persist (exclude active runs which might be large/transient)
      }),
    }
  ));
